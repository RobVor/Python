...
try:
    while True:
        t = time.time()
        tm = datetime.datetime.fromtimestamp(t).strftime('%M')
        ts = datetime.datetime.fromtimestamp(t).strftime('%S')
        th = datetime.datetime.fromtimestamp(t).strftime('%H')
        td = datetime.datetime.fromtimestamp(t).strftime('%D')
        hc595_in(HexBin[int(th)],SDI_1, SDIs=[SDI_2, HexBin[int(tm)]])
        hc595_out()
        Sec_Clock = int(ts)/int(ts)
        GPIO.output(SDI_3,1)
        time.sleep(Sec_Clock)
        GPIO.output(SDI_3,0)
        time.sleep(Sec_Clock)
except KeyboardInterrupt:
    hc595_in(HexBin[0],SDI_1, SDIs=[SDI_2, HexBin[0]])
    hc595_out()
    destroy()